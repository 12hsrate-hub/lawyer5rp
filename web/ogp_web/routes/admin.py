...
def _admin_error_response(*, status_code: int, source: str, code: str, message: str) -> JSONResponse:
    return JSONResponse(
...
async def admin_catalog_list(
    ...
    try:
        ...
        return _admin_error_response(
            status_code=status.HTTP_400_BAD_REQUEST,
            source="admin_catalog_list",
            code="admin_catalog_list_bad_request",
            message=str(exc) or "invalid_catalog_entity",
        )
    except (KeyError, PermissionError) as exc:
        return _admin_error_response(
            status_code=status.HTTP_404_NOT_FOUND,
            source="admin_catalog_list",
            code="admin_catalog_list_not_found",
            message=str(exc) or "catalog_entity_not_found",
        )
