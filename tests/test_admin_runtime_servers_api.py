...
class _FakeCatalogListWorkflowServiceError:
    def __init__(self, error: Exception):
        self.error = error

    def list_content_items(self, **kwargs):
        raise self.error
...
class AdminRuntimeServersApiTests(unittest.TestCase):
    ...
    def test_catalog_list_maps_value_error_to_400_with_structured_error(self):
        fake_workflow = _FakeCatalogListWorkflowServiceError(ValueError("bad_entity"))
        self.client.app.dependency_overrides[get_content_workflow_service] = lambda: fake_workflow
        ...
        self.assertEqual(response.status_code, 400)
        payload = response.json()
        self.assertIn("bad_entity", payload.get("detail", []))
        self.assertEqual(payload.get("error", {}).get("code"), "admin_catalog_list_bad_request")
        self.assertEqual(payload.get("error", {}).get("source"), "admin_catalog_list")
        self.assertEqual(response.headers.get("x-error-code"), "admin_catalog_list_bad_request")

    def test_catalog_list_maps_permission_error_to_404_with_structured_error(self):
        fake_workflow = _FakeCatalogListWorkflowServiceError(PermissionError("forbidden_entity"))
        self.client.app.dependency_overrides[get_content_workflow_service] = lambda: fake_workflow
        ...
        self.assertEqual(response.status_code, 404)
        payload = response.json()
        self.assertIn("forbidden_entity", payload.get("detail", []))
        self.assertEqual(payload.get("error", {}).get("code"), "admin_catalog_list_not_found")
        self.assertEqual(payload.get("error", {}).get("source"), "admin_catalog_list")
        self.assertEqual(response.headers.get("x-error-code"), "admin_catalog_list_not_found")
