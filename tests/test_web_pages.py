...
class WebPagesSmokeTests(unittest.TestCase):
    ...
    def test_admin_laws_page_contains_guided_server_setup_block(self):
        ...
        self.assertIn('href="#law-canonical-pipeline-host"', response.text)
        self.assertIn("Law Sources", response.text)
        self.assertIn("Canonical Source Sets", response.text)
        self.assertIn("Source Sets", response.text)
        self.assertIn("Source Registry", response.text)
