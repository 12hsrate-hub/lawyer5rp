...
class WebPagesSmokeTests(unittest.TestCase):
    ...
    def test_admin_laws_page_contains_guided_server_setup_block(self):
        ...
        self.assertIn("Advanced law diagnostics / compatibility seam", response.text)
        self.assertIn("Не используйте его как стартовую точку onboarding", response.text)
        ...
        self.assertIn("Чего не делать здесь", response.text)
        self.assertIn("Compatibility seam snapshot", response.text)
        self.assertIn("Compatibility-backed catalog", response.text)
