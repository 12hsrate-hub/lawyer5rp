...
class WebApiTests(unittest.TestCase):
    ...
    def test_generate_logs_pilot_runtime_shadow_compare_when_flag_enabled(self):
        previous_shadow_mode = os.environ.get("OGP_FEATURE_FLAG_PILOT_SHADOW_COMPARE_V1_MODE")
        os.environ["OGP_FEATURE_FLAG_PILOT_SHADOW_COMPARE_V1_MODE"] = "all"
        try:
            self._register_verify_and_login("shadow_compare_user", "shadow_compare_user@example.com")
            self.client.put(
                "/api/profile",
                json={
                    "name": "Rep",
            ...
            response = self.client.post(
                ...
                json={
                    ...
                    "org": "LSPD",
                    "subject_names": "John Doe",
                    "situation_description": "РћРїРёСЃР°РЅРёРµ",
                    "violation_short": "РќР°СЂСѓС€РµРЅРёРµ",
                    "event_dt": "08.04.2026 14:30",
                    "today_date": "08.04.2026",
                    "victim": {
                        "name": "Victim",
            ...
            shadow_events = [
                item
                for item in self.admin_store.backend._state["metric_events"]
                if item["event_type"] == "pilot_runtime_shadow_compare"
            ]
            self.assertTrue(shadow_events)
            latest = shadow_events[-1]
            self.assertEqual(latest["server_code"], "blackberry")
            self.assertEqual(int((latest["meta_json"] or {}).get("mismatch_count") or 0), 0)
            self.assertTrue((latest["meta_json"] or {}).get("shadow_flag_enabled"))
        finally:
            if previous_shadow_mode is None:
                os.environ.pop("OGP_FEATURE_FLAG_PILOT_SHADOW_COMPARE_V1_MODE", None)
            else:
                os.environ["OGP_FEATURE_FLAG_PILOT_SHADOW_COMPARE_V1_MODE"] = previous_shadow_mode
