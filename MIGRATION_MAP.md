...
- Accelerated runtime phase:
  ...
  - `L.runtime.30` propagate explicit shell-stage semantics through laws/workspace/issues parity summaries: runtime version parity, bridge lifecycle, activation-gap and shell-debt summaries now carry the same artifact-stage context as base health, so server-centric runtime triage can reason about `activated / active_without_projection / materialized_only / drifted` consistently instead of reconstructing that meaning from older status combinations
  - `L.runtime.31` propagate the same explicit shell-stage semantics through convergence, cutover, bridge-policy, enforcement, and risk summaries so downstream runtime governance surfaces no longer need to infer legacy-shell state from indirect status combinations
  - `L.runtime.32` propagate explicit shell-stage semantics through the remaining governance/exit-contract summaries (`governance_contract`, `legacy_path_allowance`, `exit_scorecard`, `breach_categories`, controls, gate, shrink decision, exception register, path matrix, next step, shrink sequence) so the whole runtime cutover/shrinking layer speaks the same artifact-stage language end-to-end
  - `L.runtime.33` demote `law_set` ids out of derived server-centric parity/lifecycle explanations: version-parity and lifecycle summaries now use explicit `shell_artifact_present` and artifact wording instead of falling back to `active/projected_law_set` drift text, keeping `law_set` ids confined to lower-level shell inspection paths
