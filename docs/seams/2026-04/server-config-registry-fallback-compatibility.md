# Compatibility Seam Note

- Seam ID: `seam-2026-04-server-config-registry-fallback`
- Area: `web/ogp_web/server_config/registry.py`
- Reason this seam exists: the registry still has to bridge between runtime DB server rows, bootstrap server packs, and a neutral fallback config so that new or partially onboarded servers can be addressed before the full staged onboarding path is complete.
- Current source of truth: `web/ogp_web/server_config/registry.py` resolves runtime-effective server packs and configs by preferring published DB packs, then bootstrap packs, then `_build_fallback_server_config(...)` for DB-only servers.
- Target source of truth: runtime-effective server config should come from an explicitly staged onboarding/runtime pack flow with declared readiness state, leaving bootstrap/fallback behavior as an audited exception rather than a silent runtime bridge.
- What changed in this task: `shrunk`
- Why this was necessary: the fallback seam remains active and multi-server critical, but scattered implicit `"blackberry"` defaults in app/bootstrap, pages, and synthetic/admin flows now resolve through one explicit server-config-owned default resolver instead of local ad hoc assumptions.
- Additional narrowing in the current slice: the registry now exposes one explicit runtime-resolution snapshot (`published_pack` / `bootstrap_pack` / `neutral_fallback`) that admin runtime health and server workspace surfaces consume directly, so neutral fallback remains compatible but is no longer an invisible readiness success path. DB-only servers can still resolve through the fallback for compatibility, but admin readiness now marks that state as requiring an explicit published/bootstrap runtime pack instead of presenting it as fully runtime-ready.
- Additional narrowing in the latest slice: admin server workspace and issues now expose dedicated `runtime_config_posture` and `runtime_config_debt` summaries on top of the same resolution snapshot. Operators can now distinguish `published_pack`, `bootstrap_pack`, and `neutral_fallback` as separate runtime-config states, and can see bootstrap-backed servers as medium transitional debt instead of treating config readiness as a binary green/red fallback warning only.
- Additional narrowing in the current slice: admin runtime surfaces now also expose an explicit `runtime_resolution_policy` summary, so config-path triage can classify servers as `declared_runtime`, `transitional_bootstrap`, or `compatibility_exception` instead of only showing low-level posture/debt fragments. This keeps the same resolution order at runtime, but starts expressing a stronger policy boundary for multi-server cutover planning.
- Additional narrowing in the latest slice: config-path policy is now also folded into a higher-level `runtime_bridge_policy` verdict together with law-side cutover mode. The fallback/bootstrap resolution behavior itself is unchanged, but operators no longer have to infer the policy implication of `transitional_bootstrap` or `compatibility_exception` by hand.
- Additional narrowing in the current slice: fallback/bootstrap config paths are now also checked through explicit runtime guardrails. When a server moves toward projection-preferred runtime, `runtime_operating_mode`, `runtime_policy_violations`, and `cutover_guardrails` now make fallback/bootstrap regressions visible as policy-level violations instead of leaving them as passive posture summaries only.
- Additional narrowing in the latest slice: config-path fallback/bootstrap states now also roll up into `runtime_policy_enforcement`, so compatibility-hold vs enforced projection runtime is visible as a first-class policy state instead of only as posture/debt fragments.
- Additional narrowing in the current slice: config-side fallback/bootstrap debt now also contributes to explicit `runtime_risk_register` items. This keeps fallback behavior unchanged, but it upgrades config debt from a passive posture signal into part of the server-level runtime risk picture.
- Rollback path: if onboarding or runtime server resolution regresses, continue resolving through `effective_server_pack(...)` and `_build_server_config_from_pack_or_base(...)` with the existing bootstrap/fallback ordering until readiness-state handling is introduced safely.
- Removal gate: remove or narrow the neutral fallback branch once runtime server onboarding enforces explicit readiness stages, DB-only server rows are no longer treated as runtime-addressable by default, and all active servers resolve through published/bootstrap-backed packs with declared state.
- Tests covering this seam:
  - `python -m pytest tests/test_server_config_registry.py -q`
  - `python -m pytest tests/test_server_context_service.py -q`
  - `python -m pytest tests/test_web_pages.py -q -k "verify_email_page_renders_server_context or reset_password_page_renders_server_context"`
  - `python -m pytest tests/test_web_api.py -q -k "synthetic or verify_email_page or reset_password_page"`
- Remaining risks:
  - DB-created servers can still resolve to a neutral runtime config before readiness state is explicit
  - compatibility is still preserved at runtime, so fallback-backed servers remain addressable until a stricter onboarding cutover is approved
  - the fallback keeps multi-server behavior operational, but production-ready evidence is still not auto-tracked, so admin surfaces can make onboarding explicit without proving the final state automatically
  - unrelated client-side or legacy literals may still exist outside the narrowed app/admin/pages/synthetic hotspot set
