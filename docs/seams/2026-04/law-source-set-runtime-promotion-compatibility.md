...
- What changed in this task: added the first persistence foundation for reusable `source_sets`, immutable `source_set_revisions`, and `server_source_set_bindings`, plus read-only admin surfaces that expose source sets, revisions, and server bindings, an explicit compatibility backfill action that imports current legacy flat law-source URLs into `legacy_flat` source-set revisions, and the first persisted discovery layer with `source_discovery_runs` and `discovered_law_links` plus read-only admin visibility for discovery runs and extracted law links, still without switching the active runtime law-resolution path away from the legacy flat-source compatibility model.
...
- Remaining risks:
  ...
  - discovery execution remains manual/test-seeded; no canonical discovery job or adapter layer exists yet
