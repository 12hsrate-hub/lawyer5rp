...
- What changed in this task: added the first persistence foundation for reusable `source_sets`, immutable `source_set_revisions`, and `server_source_set_bindings`, plus read-only admin surfaces that expose source sets, revisions, and server bindings, an explicit compatibility backfill action that imports current legacy flat law-source URLs into `legacy_flat` source-set revisions, the first persisted discovery layer with `source_discovery_runs` and `discovered_law_links`, including a deterministic manual discovery execution path for `legacy_flat` and passthrough-compatible revisions plus admin visibility for discovery runs and extracted law links, and an additive canonical law-document foundation (`canonical_law_documents` plus aliases) with bounded ingest/read surfaces over discovery runs, still without switching the active runtime law-resolution path away from the legacy flat-source compatibility model or replacing the existing runtime `law_documents` projection table.
