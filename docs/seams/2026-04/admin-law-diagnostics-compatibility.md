...
- Current source of truth: `/admin/servers` and `/admin/servers/{server_code}` are the primary operator workspace for server setup and maintenance; `/admin/ops` and `/admin/audit` are the canonical secondary global entrypoints; `/admin/laws` remains a secondary compatibility and diagnostics surface only.
